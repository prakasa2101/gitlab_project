...
module "aurora" {
  ...
  name = each.value.aurora_cluster_name
  ...
  kms_key_id        = var.kms_keys["rds-aurora"].arn
  ...
  cloudwatch_log_group_kms_key_id        = var.kms_keys["cloudwatch-logs"].arn
  ...
  db_cluster_parameter_group_name        = each.value.aurora_cluster_name
  db_cluster_parameter_group_family      = data.aws_rds_engine_version.aurora.parameter_group_family
  db_cluster_parameter_group_description = "${each.value.aurora_cluster_name} cluster parameter group"
...
output "writer_endpoints" {
  value = values(module.aurora).*.cluster_endpoint
}
