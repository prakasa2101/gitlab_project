module "eks_managed_node_group" {
  ...
  name                 = "${var.project_name}-${var.environment}-${index(var.private_subnets, each.key)}"
  use_name_prefix      = true
  iam_role_name        = "${var.project_name}-${var.environment}-${index(var.private_subnets, each.key)}"
  ...
  tags = merge(local.tags, {
    "autoscaling_group"    = "${var.project_name}-${var.environment}-${index(var.private_subnets, each.key)}"
    "autoscalinggroupname" = "${var.project_name}-${var.environment}-${index(var.private_subnets, each.key)}"
  })
