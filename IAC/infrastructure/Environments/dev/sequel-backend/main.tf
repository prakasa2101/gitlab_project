...
backend "s3" {
  ...
  key            = "account/environments/dev/ct-backend-terraform.tfstate"
  region         = "us-east-1"        
  encrypt        = true
...
resource "aws_security_group" "ecs-sg" {
  ...
  description = "opt ct backend security group"
...
resource "aws_security_group" "lb-sg" {
  ...
  description = "opt patient creation be lb security group"
  vpc_id      = data.terraform_remote_state.previous_state.outputs.vpc-id
